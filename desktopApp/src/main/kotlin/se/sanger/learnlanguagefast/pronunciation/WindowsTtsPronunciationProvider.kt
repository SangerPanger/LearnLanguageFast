    ...
    override suspend fun generate(text: String, languageCode: String): ByteArray? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val psScript = buildString {
                ...
                // Try exact culture, then language prefix, then any enabled voice
                append("\$voicesExact = \$s.GetInstalledVoices() | Where-Object { \$_.Enabled -and \$_.VoiceInfo.Culture.Name -eq $langPs }; ")
                append("if (-not \$voicesExact -or \$voicesExact.Count -eq 0) { ")
                append("  \$langPrefix = ($langPs -split '-') [0]; ")
                append("  \$voicesLang = \$s.GetInstalledVoices() | Where-Object { \$_.Enabled -and \$_.VoiceInfo.Culture.Name -like (\$langPrefix + '-*') }; ")
                append("} else { \$voicesLang = @() } ")
                append("\$voicesAny = \$s.GetInstalledVoices() | Where-Object { \$_.Enabled }; ")
                append("if ((-not \$voicesExact -or \$voicesExact.Count -eq 0) -and (-not \$voicesLang -or \$voicesLang.Count -eq 0)) { ")
                append("  Write-Output \"[WindowsTTS][WARN] No exact or language-matched voice for $langPs. Falling back to any enabled voice.\"; ")
                append("  \$v = \$voicesAny[0]; ")
                append("} elseif (-not \$voicesExact -or \$voicesExact.Count -eq 0) { ")
                append("  Write-Output \"[WindowsTTS][WARN] No exact voice for $langPs. Using language-matched voice.\"; ")
                append("  \$v = \$voicesLang[0]; ")
                append("} else { \$v = \$voicesExact[0]; } ")
